...
class HttpResponseHelperSpec extends AnyFlatSpec with Matchers with ScalaCheckDrivenPropertyChecks {
  ...
  "handleErrorResponse" should "transform Bad Request into BadRequestException" in {
    ...
    fixture()(response) shouldBe MpeError("BAD_REQUEST",
      "test-method of 'test-url' returned 400 (Bad Request). Response body 'Message for 400'", None)
  }

  it should "transform Not Found into NotFoundException" in {
    val response = responseFor(NOT_FOUND)
    fixture()(response) shouldBe MpeError("NOT_FOUND",
      "test-method of 'test-url' returned 404 (Not Found). Response body: 'Message for 404'", None)
  }
  ...
  it should "transform any other 4xx into Upstream4xxResponse" in {
    ...
    forAll(userErrors) {
      userError =>
        ...
        ex shouldBe MpeError("FORBIDDEN",
          s"test-method of 'test-url' returned $userError. Response body: 'Message for $userError'", None)
    }
  }
  ...
  it should "transform any 5xx into Upstream5xxResponse" in {
    ...
    forAll(serverErrors) {
      serverError =>
        ...
        ex shouldBe MpeError("INTERNAL_ERROR",
          s"test-method of 'test-url' returned $serverError. Response body: 'Message for $serverError'", None)
    }
  }
